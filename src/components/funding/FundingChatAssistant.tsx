...
interface FundingChatAssistantProps {
  context?: {
    projectTitle?: string;
    budgetRange?: string;
    timeline?: string;
...
const CHAT_URL = `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/funding-assistant`;
...
export function FundingChatAssistant({ context }: FundingChatAssistantProps) {
  ...
  return (
    <div className="bg-gradient-to-br from-emerald-500/10 to-emerald-500/5 border-2 border-emerald-500/30 rounded-lg overflow-hidden">
      <Collapsible open={isOpen} onOpenChange={setIsOpen}>
        <CollapsibleTrigger className="w-full flex items-center justify-between p-4 hover:bg-emerald-500/10 transition-colors">
          <div className="flex items-center gap-2">
            <div className="p-1.5 bg-emerald-500/20 rounded-lg">
              <MessageCircle className="h-5 w-5 text-emerald-500" />
            </div>
            <div className="text-left">
              <span className="font-semibold text-foreground block">Funding Assistant</span>
              <span className="text-xs text-muted-foreground">Ask about grants, investors, crowdfunding...</span>
            </div>
          </div>
          <ChevronDown 
                ...
                {messages.map((msg, i) => (
                  <div
                    ...
                    className={`text-sm rounded-lg p-3 ${
                      msg.role === "user"
                        ? "bg-emerald-500 text-white ml-6"
